Shader "LayaAir3D/PBR(Specular)" {
	Properties {
		_MainTex("Diffuse Texture", 2D) = "white" {}
		_Color("Diffuse Color", Color) = (1,1,1,1)

		_SpecGlossMap("Metallic", 2D) = "white" {}
		_SpecColor("Diffuse Color", Color) = (0.2,0.2,0.2,1)
		...
		[ToggleOff] _Emission("Emission", Float) = 0.0
	...
	SubShader {
		...
		Pass { 
			...
			#include "../CGIncludes/LayaPBRSpecularFB.cginc"
		...
		Pass { 
			...
			#include "../CGIncludes/LayaPBRSpecularFA.cginc"
			
			ENDCG
		}
		
	}
	CustomEditor "LayaPBRSpecularGUI"
	FallBack "Standard"
}
