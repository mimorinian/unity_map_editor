...
fixed4 frag(v2f i) : SV_Target {
	...
	diffuse = (diffuse + UNITY_LIGHTMODEL_AMBIENT + lightMapColor) * albedo;
	...
	fixed4 color = fixed4((diffuse + specular).rgb * atten * shadow, albedo.a);

	UNITY_APPLY_FOG(i.fogCoord, color);

	color = clamp(color, 0, 1);

	return fixed4((diffuse + specular).rgb * atten * shadow, albedo.a);
}
